...
cb.type="checkbox";cb.checked=p.done;
...
(data[dia].eventos||[]).forEach((e,i)=>{
let div=document.createElement("div");div.className="evento";

let cb=document.createElement("input");
cb.type="checkbox";cb.checked=e.done;
...
if(p!=null){
if(p<40)div.classList.add("rojo");
else if(p<80)div.classList.add("amarillo");
...
r.oninput=()=>{
let data=getData();
data[getDia()].biblia.reflexion=r.value;
saveData(data);
};
}
