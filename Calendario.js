/* ============================================================
   MODO ÁGUILA — calendario.js v3.0
   Fechas futuras activas · Agregar citas · Checkboxes · Merge agenda
   ============================================================ */
'use strict';
...
function dKey(y,m,d){
  /* Use Date to handle month overflow correctly */
  const dt=new Date(y,m,d);
  return `${dt.getFullYear()}-${String(dt.getMonth()+1).padStart(2,'0')}-${String(dt.getDate()).padStart(2,'0')}`;
}

/* Merge calendar + main agenda events */
function getEventsForKey(key){
  const calData =loadCal();
  const mainData=loadMain();
...
function renderMes(){
  ...
  for(let i=startDow-1;i>=0;i--){
    ...
    html+=`<div class="cal-cell other${hasEvents(k)?' has':''}" onclick="selectDay('${k}')">${d2}</div>`;
  }
  ...
  for(let d2=1;d2<=daysInMonth;d2++){
    ...
    const cellDate=new Date(Y,M,d2);
    const isFuture=cellDate>today&&!isToday;
    const cls=[isToday?'today':'',isSel?'sel':'',isFuture?'future':'',hasEvents(k)?'has':''].filter(Boolean).join(' ');
    html+=`<div class="cal-cell ${cls}" onclick="selectDay('${k}')">${d2}</div>`;
  }
...
/* Now accepts a pre-formatted key string directly — no overflow issues */
function selectDay(key){
  selKey=key;

  /* Navigate calendar to the month of selected day if needed */
  const[Y,M]=key.split('-').map(Number);
  if(M-1!==navDate.getMonth()||Y!==navDate.getFullYear()){
    navDate=new Date(Y,M-1,1);
  }
...
function renderSemana(){
  ...
  for(let i=0;i<7;i++){
    ...
    html+=`
      <div class="week-row${isToday?' today':''}${isSel?' sel':''}" onclick="selectDay('${k}')">
        <div class="week-row-meta">
          <div class="week-row-dow">${DOWS[i]}</div>
          <div class="week-row-d">${d.getDate()}</div>
        </div>
...
function openDay(key){
  ...
  let flag='';
  if(isToday) flag=' <span style="font-size:.7rem;color:var(--amber);margin-left:6px">HOY</span>';
  if(isFuture)flag=' <span style="font-size:.7rem;color:var(--green);margin-left:6px">FUTURO</span>';
  if(isPast)  flag=' <span style="font-size:.7rem;color:var(--t3);margin-left:6px">PASADO</span>';

  if(title)title.innerHTML=`${DAYS_FULL[date.getDay()]}, ${D} de ${MONTHS[M-1]} ${Y}${flag}`;

  /* Time default */
  const ti=q('#evTime');
  if(ti){
    if(isToday){const n=new Date();ti.value=`${String(n.getHours()).padStart(2,'0')}:${String(n.getMinutes()).padStart(2,'0')}`;}
    else ti.value='09:00';
  }
...
function renderDayEvList(key){
  ...
  if(!evs.length){
    list.innerHTML='<div class="empty-msg" style="padding:14px 0"><span class="emi" style="font-size:1.4rem">📭</span>Sin eventos — agrega uno abajo</div>';
    return;
  }

  list.innerHTML=evs.map(ev=>`
    <div class="day-ev${ev.done?' done':''}" data-evid="${ev.id}">
      <button class="day-ev-chk" onclick="toggleCalEv('${key}',${ev.id})" aria-label="${ev.done?'Desmarcar':'Marcar'}">${ev.done?'✓':''}</button>
      <span class="day-ev-time">${ev.time||'—'}</span>
      <span class="day-ev-text">${esc(ev.text)}</span>
      ${ev._main
        ?'<span style="font-size:.68rem;color:var(--t3);flex-shrink:0;padding:2px 6px;background:var(--bg-hover);border-radius:4px">agenda</span>'
        :`<button class="ico-btn del" onclick="deleteCalEv('${key}',${ev.id})" title="Eliminar">✕</button>`
      }
    </div>`).join('');
}
...
/* ============================================================
   ...
   ============================================================ */
function addCalEv(){
  if(!selKey){toast('⚠️ Selecciona un día primero','warn');return;}
...
function toggleCalEv(key,id){
  ...
  if(ev){ev.done=!ev.done;saveCal(data);renderDayEvList(key);
    if(ev.done)toast('✅ Completado','ok');
  }
}
